...
class Atom { 
  ...
  public Atom (Element e, PVector vel, PVector pos, float diameter) {
    ...
    for(int i = 0; i < this.total; i++){
      PVector p = pos.copy().add(pv(random(-diameter*total/2,diameter*total/2),random(-diameter*total/2,diameter*total/2)));
  ...
  public void display() {
    ...
    textSize(20);
    fill(255);
    PVector apc = convert(avgPos);
    switch(e) {
      case H: text("H", apc.x, apc.y, 10, 10);break;
      case He: text("He", apc.x, apc.y, 10, 10);break;
      case C: text("C", apc.x, apc.y, 10, 10);break;
      case Na: text("Na", apc.x, apc.y, 10, 10);break;
      case Si: text("Si", apc.x, apc.y, 10, 10);break;
      case Fe: text("Fe", apc.x, apc.y, 10, 10);break;
    }
  }
